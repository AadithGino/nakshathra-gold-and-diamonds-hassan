...
export async function listStaff(
  ...
  const baseQuery = StaffProfile.find(filter).populate(
    "userId",
    "name phone status lastLoginAt",
  );
...
export async function getStaffDetails(id: string, from?: Date, to?: Date) {
  ...
  const [populatedProfile, report, payments, submissions, corrections] =
    await Promise.all([
      StaffProfile.findById(profile._id)
...
export const listStaffCorrections = (staffId: string) =>
  PaymentCorrection.find({ requestedBy: staffId })
...
export const listStaffCashSubmissions = (
  staffId: string,
  from?: Date,
  to?: Date,
) =>
...
export const getStaffProfile = (staffId: string) =>
  StaffProfile.findOne({ userId: staffId })
    .populate("userId", "name phone lastLoginAt")
    .lean();
...
export async function getStaffReceipt(staffId: string, paymentId: string) {
  const payment = await Payment.findOne({
    ...
    .lean();
  if (!payment)
    throw new AppError("RECEIPT_NOT_FOUND", "Receipt not found", 404);
